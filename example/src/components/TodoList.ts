...
class TodoItemState {
	constructor(
		public text: string,
		public done: State<boolean>,
		public deleted: State<boolean>,
	) {}
	serialize() {
		return { text: this.text, done: this.done.val };
	}
}

const TodoItem =
	({ text, done, deleted }: TodoItemState) =>
	() =>
		deleted.val
			? null
			: div(
...
class TodoListState {
	...
	static readonly load = () =>
		new TodoListState(
			JSON.parse(localStorage.getItem("appState") ?? "[]").map(
				(t: any) => new TodoItemState(t.text, van.state(t.done), van.state(false)),
			),
		);
